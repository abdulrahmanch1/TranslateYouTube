...
export default function Signup() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (userId) {
      // Ensure a profile row exists (robust even if DB trigger missing)
      try { await (supa as any).rpc('ensure_profile') } catch {}
      // Ensure a wallet exists (init tx row) even if triggers are missing
      try { await (supa as any).rpc('ensure_wallet') } catch {}
      // The DB trigger already creates the profile row. Update only to avoid insert conflicts.
      await (supa
    ...
    <div className="px-6 sm:px-10 py-16">
      <div className="max-w-lg mx-auto glass rounded-2xl p-6">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <input type="text" placeholder="Full name" value={name} onChange={e=>setName(e.target.value)} className="w-full rounded-lg bg-white/5 border border-white/10 px-4 py-3" />
          ...
          <button disabled={loading} className="w-full rounded-lg bg-primary-500 hover:bg-primary-400 px-4 py-3 font-medium">{loading ? 'Creating…' : 'Create account'}</button>
